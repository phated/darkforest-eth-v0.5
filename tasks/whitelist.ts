...
task("whitelist:changeDrip", "change the faucet amount for whitelisted players")
  .addPositionalParam(
    "value",
    "drip value (in ether or xDAI)",
    undefined,
    types.float
  )
  .setAction(changeDrip);

async function changeDrip(
  args: { value: number },
  ...
  const txReceipt = await contract.changeDrip(
    hre.ethers.utils.parseEther(args.value.toString())
  );
...
async function whitelistExistsAddress(
  args: { address: string },
  hre: HardhatRuntimeEnvironment
) {
  await hre.run("utils:assertChainId");
  ...
  console.log(
    `Player ${args.address} is${isWhitelisted ? "" : " NOT"} whitelisted.`
  );
}
...
async function whitelistExistsKeyHash(
  args: { key: string },
  hre: HardhatRuntimeEnvironment
) {
  await hre.run("utils:assertChainId");
