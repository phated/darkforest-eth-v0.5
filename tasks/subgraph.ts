...
task(
  TASK_SUBGRAPH_CODEGEN,
  "generate subgraph files before graph deploy"
).setAction(subgraphCodegen);

async function subgraphCodegen(
  _args: HardhatArguments,
  ...
  const warning =
    "THIS FILE IS GENERATED DO NOT EDIT THIS FILE. EDIT subgraph.template.yaml\n\n";
  const yaml = (
    await fs.readFile(path.join(subgraphPath, "subgraph.template.yaml"))
  )
    .toString()
  ...
  await fs.writeFile(
    path.join(subgraphPath, "subgraph.yaml"),
    "# " + warning + yaml
  );
...
async function subgraphDeploy(
  ...
  console.log(
    "or the usual docker container ps and docker container stop and docker container rm"
  );
}
...
task(TASK_SUBGRAPH_CLEANUP, "shutdown graph dockers").setAction(
  subgraphCleanup
);
...
async function subgraphCleanup() {
  ...
  const leftovers: DockerodeProcess[] = compose.map((svc) => {
    if (!svc.name || !svc.Image) {
      throw new HardhatPluginError(
        PLUGIN_NAME,
        "type undefiend on DFContainerCreateOptions"
      );
    }
  ...
  for (const svc of compose) {
    ...
    try {
      ...
      if (svc.stdout && hre.hardhatArguments.verbose) {
        const stream = await p.container.logs({
          follow: true,
          stdout: true,
          stderr: true,
        });
      ...
      throw new HardhatPluginError(
        PLUGIN_NAME,
        `Unable to create service: ${svc.name}`
      );
    }
  }
...
const compose: DFContainerCreateOptions[] = [
  ...
  },
  {
    ...
    Env: [
      ...
      "ipfs=ipfs:5001",
      `ethereum=xdai:http://${
        isLinux ? "172.17.0.1" : "host.docker.internal"
      }:8545`,
      "RUST_LOG=info",
      "GRAPH_LOG=debug",
    ],
