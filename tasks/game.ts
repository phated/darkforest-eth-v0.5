...
async function setPlanetTransferEnabled(
  ...
  const setTransferEnabledReceipt = await contract.setPlanetTransferEnabled(
    args.enabled
  );
...
task("game:setWorldRadiusMin", "change the WORLD_RADIUS_MIN")
  .addPositionalParam(
    "radius",
    "the minimum radius of the world",
    undefined,
    types.int
  )
  .setAction(gameSetWorldRadiusMin);

async function gameSetWorldRadiusMin(
  args: { radius: number },
  hre: HardhatRuntimeEnvironment
) {
  await hre.run("utils:assertChainId");
  ...
  const changeWorldRadiusMinReceipt = await contract.changeWorldRadiusMin(
    args.radius
  );
...
async function setTokenMintEnd(
  args: { tokenend: number },
  hre: HardhatRuntimeEnvironment
) {
  await hre.run("utils:assertChainId");
...
task(
  "game:setPlanetOwner",
  "sets the owner of the given planet to be the given address"
)
  .addPositionalParam(
    "planetId",
    "non-0x-prefixed planet locationId",
    undefined,
    types.string
  )
  .addPositionalParam(
    "address",
    "0x-prefixed address of a player",
    undefined,
    types.string
  )
...
async function setPlanetOwner(
  ...
  const setPlanetOwnerReciept = await contract.setOwner(
...
task(
  "game:findCheaters",
  "finds planets that have been captured more than once"
).setAction(findCheaters);
...
async function findCheaters({}, hre: HardhatRuntimeEnvironment) {
  ...
  const scoreMap = (await contract.getGameConstants())
  ...
  for (let i = 0; i < logs.length; i++) {
    ...
    if (
      hexLocationId ===
      "0000c1710074979bb76b36b238e68297e02839cb0452f0dad517517cda42e9d4"
    ) {
  ...
  for (const [cheater, score] of cheaterScore.entries()) {
    console.log(
      `Player ${cheater} earned ${score.toLocaleString()} score from cheating.`
    );
...
async function createPlanets({}, hre: HardhatRuntimeEnvironment) {
  ...
  for (const adminPlanetInfo of hre.settings.darkforest.adminPlanets) {
    try {
      ...
      console.log(
        `created admin planet at (${adminPlanetInfo.x}, ${adminPlanetInfo.y})`
      );
    } catch (e) {
      console.log(
        `error creating planet at (${adminPlanetInfo.x}, ${adminPlanetInfo.y}):`
      );
      console.log(e);
    }
  }
